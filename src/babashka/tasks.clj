...
(def ^:private default-log-level :error)
(def ^:dynamic *log-level* default-log-level)
;; (def task-name (sci/new-dynamic-var '*-task-name* nil {:ns sci-ns}))
(def ^:dynamic *task* nil)
(defn current-task [] *task*)
(def state {})
...
(defn shell [& args]
  (let [{:keys [prev cmd opts]} (p/parse-args args)
    ...
    (binding [*log-level* (or local-log-level *log-level*)]
      (apply log-info args)
      (handle-non-zero (p/process* {:opts opts :cmd cmd :prev prev}) opts))))
...
(defn clojure [& args]
  (let [[cmd & args] args
    ...
    (binding [*log-level* (or local-log-level *log-level*)]
      ...
      #_(handle-non-zero (apply deps/clojure (merge default-opts opts) cmd) opts))))
...
(defn exec
  ([sym]
   (let [snippet (exec-fn-snippet sym)]
     (load-string snippet)))
  ([sym extra-opts]
   (let [snippet (exec-fn-snippet sym extra-opts)]
     (load-string snippet))))

(defn -main [task & args]
  (reset! bb-edn (edn/read-string (slurp "tasks.edn")))
